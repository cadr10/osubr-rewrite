...
// The React Component
const DifficultyIcon: React.FC<DifficultyIconProps> = ({ diff, small = false }) => {
  const color = getStarColor(diff.stars);

  const svgProps = {
    xmlns: "http://www.w3.org/2000/svg",
    width: small ? "1em" : "1.2em", 
    height: small ? "1em" : "1.2em",
    viewBox: "0 0 32 32",
    "aria-hidden": true,
  };
  ...
  return (
    <div className={`difficulty-container ${small ? 'small' : ''}`} title={`${diff.stars.toFixed(2)} stars`}>
      {renderIcon()}
      <span className="star-rating">{diff.stars.toFixed(2)}</span>
      <style jsx>{`
        ...
        /* Styles for the smaller version */
        .difficulty-container.small {
          gap: 4px;
          padding: 2px 5px;
          font-size: 11px;
          border-radius: 3px;
